		...
		Spec: computev1.PolarsClusterSpec{
			Runtime: &computev1.RuntimeSpec{
				Composed: computev1.ComposedRuntimeSpec{
					Dist:         &computev1.ImageSpec{Tag: testDistTag},
...
func TestDefaultVersionIsSupported(t *testing.T) {
	...
	v, err := utilversion.ParseSemantic(computev1.DefaultVersion)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(v.LessThan(minSupportedVersion)).To(BeFalse(),
		"the CRD default version must not be below the operator's supported minimum")
}
...
func TestClusterVersion(t *testing.T) {
	...
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(v).To(BeNil(), "unset version parses to nil")

	cluster.Spec.Version = testDistTag
	v, err = clusterVersion(cluster)
	g.Expect(err).NotTo(HaveOccurred())
